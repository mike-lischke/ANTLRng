...
public  testLL1():  void {
		...
		let  info = this.interpAndGetDecisionInfo(TestParserProfiler.lg, g, "s", ";");
...
public  testLL2():  void {
		...
		let  info = this.interpAndGetDecisionInfo(TestParserProfiler.lg, g, "s", "xyz;");
		assertEquals(1, info.length);
		let  expecting =
...
public  testRepeatedLL2():  void {
		...
		let  info = this.interpAndGetDecisionInfo(TestParserProfiler.lg, g, "s", "xyz;", "abc;");
		assertEquals(1, info.length);
		let  expecting =
...
public  test3xLL2():  void {
		...
		let  info = this.interpAndGetDecisionInfo(TestParserProfiler.lg, g, "s", "xyz;", "abc;", "z.");
		assertEquals(1, info.length);
		let  expecting =
...
public  testOptional():  void {
		...
		let  info = this.interpAndGetDecisionInfo(TestParserProfiler.lg, g, "s", "a.b;");
...
public  test2xOptional():  void {
		...
		let  info = this.interpAndGetDecisionInfo(TestParserProfiler.lg, g, "s", "a.b;", "a.b;");
		assertEquals(2, info.length);
		let  expecting =
			"[{decision=0, contextSensitivities=0, errors=0, ambiguities=0, SLL_lookahead=2, " +
...
public  testContextSensitivity():  void {
		let  g = new  Grammar(
			...
			"e : INT | ;\n",
			TestParserProfiler.lg);
		let  info = this.interpAndGetDecisionInfo(TestParserProfiler.lg, g, "a", "; 1 x");
		assertEquals(2, info.length);
		let  expecting =
...
public  testSimpleLanguage():  void {
		let  g = new  Grammar(TestXPath.grammar);
		let  input =
			"def f(x,y) { x = 3+4*1*1/5*1*1+1*1+1; y; ; }\n" +
			"def g(x,a,b,c,d,e) { return 1+2*x; }\n"+
			"def h(x) { a=3; x=0+1; return a*x; }\n";
		let  info = this.interpAndGetDecisionInfo(g.getImplicitLexer(), g, "prog", input);
		let  expecting =
...
public  testDeepLookahead():  void {
		...
		let  info = this.interpAndGetDecisionInfo(TestParserProfiler.lg, g, "s", "a+b+c;");
...
public  testProfilerGeneratedCode():  void {
		...
		let  runOptions = createOptionsForJavaToolTests("T.g4", grammar, "TParser", "TLexer",
...
try {
	try  {
			let  state =  runner.run(runOptions) as ExecutedState;
			let  expecting =
			...
			startRule: string,... input: string[]):  DecisionInfo[]
	{
		...
		let  lexEngine = lg.createLexerInterpreter(null);
		let  parser = g.createParserInterpreter(null);
		parser.setProfile(true);
		for (let s of ANTLRInputStream.ANTLRInputStream.input) {
			lexEngine.reset();
			parser.reset();
			lexEngine.setInputStream(new  ANTLRInputStream(s));
			let  tokens = new  CommonTokenStream(lexEngine);
			parser.setInputStream(tokens);
			let  r = g.rules.get(startRule);
			if ( r===null ) {
