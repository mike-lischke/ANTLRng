...
export class TokenStreamRewriter extends JavaObject {
    ...
    public RewriteOperation = (($outer) => {
        return class RewriteOperation extends JavaObject {
            ...
            public toString = (): java.lang.String | null => {
                ...
                const $index: number = opName.indexOf("$");
    ...
    public rollback(instructionIndexOrProgramName: number | java.lang.String | null, instructionIndex?: number): void {
        if (typeof instructionIndexOrProgramName === "number" && instructionIndex === undefined) {
            const instructionIndex = instructionIndexOrProgramName;
            this.rollback(TokenStreamRewriter.DEFAULT_PROGRAM_NAME, instructionIndex);
        } else {
            const programName = instructionIndexOrProgramName as java.lang.String;
            const is: java.util.List<RewriteOperation> = this.programs.get(programName);
            if (is !== null) {
    ...
    public insertAfter(tOrIndexOrProgramName: Token | number | java.lang.String | null, textOrTOrIndex: java.lang.Object | Token | number | null, text?: java.lang.Object | null): void {
        if (tOrIndexOrProgramName instanceof Token && textOrTOrIndex instanceof java.lang.Object && text === undefined) {
            ...
            const index = tOrIndexOrProgramName;
            const text = textOrTOrIndex;
            ...
            const programName = tOrIndexOrProgramName;
            ...
            const index = textOrTOrIndex as number;
            // to insert after, just insert before next index (even if past end)
            const op: RewriteOperation = new InsertAfterOp(index, text);
            const rewrites: java.util.List<RewriteOperation> = this.getProgram(programName);
    ...
    public insertBefore(tOrIndexOrProgramName: Token | number | java.lang.String | null, textOrTOrIndex: java.lang.Object | Token | number | null, text?: java.lang.Object | null): void {
        if (tOrIndexOrProgramName instanceof Token && textOrTOrIndex instanceof java.lang.Object && text === undefined) {
            ...
            const programName = tOrIndexOrProgramName as java.lang.String;
            const index = textOrTOrIndex as number;
            const op: RewriteOperation = new InsertBeforeOp(index, text);
            const rewrites: java.util.List<RewriteOperation> = this.getProgram(programName);
            op.instructionIndex = rewrites.size();
            rewrites.add(op);
        }
    ...
    public replace(indexOrIndexTOrFromOrProgramName: number | Token | java.lang.String | null, textOrToOrFrom: java.lang.Object | number | Token | null, textOrTo?: java.lang.Object | number | Token | null, text?: java.lang.Object | null): void {
        if (typeof indexOrIndexTOrFromOrProgramName === "number" && textOrToOrFrom instanceof java.lang.Object && textOrTo === undefined) {
            const index = indexOrIndexTOrFromOrProgramName;
            const text = textOrToOrFrom;
            ...
            const from = indexOrIndexTOrFromOrProgramName;
            const to = textOrToOrFrom;
            const text = textOrTo;
            ...
            const programName = indexOrIndexTOrFromOrProgramName;
            const from = textOrToOrFrom;
            const to = textOrTo;
            if (from > to || from < 0 || to < 0 || to >= this.tokens.size()) {
                throw new java.lang.IllegalArgumentException(S`replace: range invalid: ` + from + S`..` + to + S`(size=` + this.tokens.size() + S`)`);
            }
            const op: RewriteOperation = new ReplaceOp(from, to, text);
            const rewrites: java.util.List<RewriteOperation> = this.getProgram(programName);
            op.instructionIndex = rewrites.size();
            rewrites.add(op);
        } else {
            const programName = indexOrIndexTOrFromOrProgramName as java.lang.String;
            const from = textOrToOrFrom as Token;
            const to = textOrTo as Token;
    ...
    public getLastRewriteTokenIndex(programName?: java.lang.String | null): number {
        if (programName === undefined) {
            ...
            const I: java.lang.Integer = this.lastRewriteTokenIndexes.get(programName);
            if (I === null) {
                return -1;
            }
    ...
    private initializeProgram = (name: java.lang.String | null): java.util.List<RewriteOperation> | null => {
        const is: java.util.List<RewriteOperation> = new java.util.ArrayList<RewriteOperation>(TokenStreamRewriter.PROGRAM_INIT_SIZE);
    ...
    public getText(programNameOrInterval?: java.lang.String | Interval | null, interval?: Interval | null): java.lang.String | null {
        if (programNameOrInterval === undefined) {
            ...
            const programName = programNameOrInterval;
            ...
            const interval = programNameOrInterval;
            ...
            const rewrites: java.util.List<RewriteOperation> = this.programs.get(programName);
            ...
            const indexToOp: java.util.Map<java.lang.Integer, RewriteOperation> = this.reduceToSingleOperationPerIndex(rewrites);

            // Walk buffer, executing instructions and emitting tokens
            let i: number = start;
            while (i <= stop && i < this.tokens.size()) {
                const op: RewriteOperation = indexToOp.get(i);
    ...
    protected reduceToSingleOperationPerIndex = (rewrites: java.util.List<RewriteOperation> | null): java.util.Map<java.lang.Integer, RewriteOperation> | null => {
        ...
        for (let i = 0; i < rewrites.size(); i++) {
            const op: RewriteOperation = rewrites.get(i);
            if (op === null) {
                continue;
            }
            ...
            const rop: ReplaceOp = rewrites.get(i) as ReplaceOp;
            // Wipe prior inserts within range
            const inserts: java.util.List<InsertBeforeOp> = this.getKindOfOps(rewrites, InsertBeforeOp.class, i);
            ...
            const prevReplaces: java.util.List<ReplaceOp> = this.getKindOfOps(rewrites, ReplaceOp.class, i);
        ...
        for (let i = 0; i < rewrites.size(); i++) {
            ...
            const iop: InsertBeforeOp = rewrites.get(i) as InsertBeforeOp;
            // combine current insert with prior if any at same index
            const prevInserts: java.util.List<InsertBeforeOp> = this.getKindOfOps(rewrites, InsertBeforeOp.class, i);
        ...
        const m: java.util.Map<java.lang.Integer, RewriteOperation> = new java.util.HashMap<java.lang.Integer, RewriteOperation>();
        for (let i = 0; i < rewrites.size(); i++) {
            const op: RewriteOperation = rewrites.get(i);
            if (op === null) {
                continue;
    ...
    protected getKindOfOps = <T extends RewriteOperation>(rewrites: java.util.List<RewriteOperation> | null, kind: java.lang.Class<T> | null, before: number): java.util.List<T> | null => {
        const ops: java.util.List<T> = new java.util.ArrayList<T>();
