...
export class PredictionMode extends Enum<PredictionMode> {
     ...
     * @returns tbd
     */
    public static hasSLLConflictTerminatingPrediction = (mode: PredictionMode, configs: ATNConfigSet): boolean => {
        ...
        const heuristic =
            PredictionMode.hasConflictingAltSet(altSets) && !PredictionMode.hasStateAssociatedWithOneAlt(configs);
    ...
    public static allSubsetsEqual = (altSets: java.util.Collection<java.util.BitSet>): boolean => {
        const it = altSets.iterator();
        const first: java.util.BitSet = it.next();
        while (it.hasNext()) {
            const next: java.util.BitSet = it.next();
     ...
     * in {@code altSets}.
     *
     * @param altSetsOrConfigs a collection of alternative subsets
      @returns the set of represented alternatives in {@code altSets}
     */
    public static getAlts(altSetsOrConfigs: java.util.Collection<java.util.BitSet> | ATNConfigSet): java.util.BitSet {
        if (altSetsOrConfigs instanceof ATNConfigSet) {
            const configs = altSetsOrConfigs;
            const alts: java.util.BitSet = new java.util.BitSet();
            for (const config of configs) {
                alts.set(config.alt);
            }
            ...
            const altSets = altSetsOrConfigs;
            const all: java.util.BitSet = new java.util.BitSet();
            for (const alts of altSets) {
                all.or(alts);
            }
     ...
     * @returns tbd
     */
    public static getConflictingAltSubsets = (configs: ATNConfigSet): java.util.Collection<java.util.BitSet> => {
        const configToAlts = new PredictionMode.AltAndContextMap();
        for (const c of configs) {
            let alts = configToAlts.get(c);
            if (alts === null) {
                alts = new java.util.BitSet();
     ...
     * @returns tbd
     */
    public static getStateToAltMap = (configs: ATNConfigSet): java.util.Map<ATNState, java.util.BitSet> => {
        const m = new java.util.HashMap<ATNState, java.util.BitSet>();
        for (const c of configs) {
    ...
    public static hasStateAssociatedWithOneAlt = (configs: ATNConfigSet): boolean => {
        const x: java.util.Map<ATNState, java.util.BitSet> = PredictionMode.getStateToAltMap(configs);
    ...
    public static getSingleViableAlt = (altSets: java.util.Collection<java.util.BitSet>): number => {
        const viableAlts = new java.util.BitSet();
        for (const alts of altSets) {
