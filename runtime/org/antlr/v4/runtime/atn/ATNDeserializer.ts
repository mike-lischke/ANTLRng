...
export class ATNDeserializer {
    ...
    public deserialize(data: number[]): ATN {
        if () {
            ...
            if (version !== ATNDeserializer.SERIALIZED_VERSION) {
                let reason: string = java.lang.StringBuilder.format(java.util.Locale.getDefault(), "Could not deserialize ATN with version %d (expected %d).", version, ATNDeserializer.SERIALIZED_VERSION);
    ...
    protected stateFactory = (type: number, ruleIndex: number): ATNState => {
        ...
        switch (type) {
            ...
            default:
                let message: string = java.lang.StringBuilder.format(java.util.Locale.getDefault(), "The specified state type %d is not valid.", type);
    ...
    protected lexerActionFactory = (type: LexerActionType, data1: number, data2: number): LexerAction => {
        switch (type) {
            ...
            default:
                throw new java.lang.IllegalArgumentException(java.lang.StringBuilder.format(java.util.Locale.getDefault(), "The specified lexer action type %s is not valid.", type));
        }
    }
