...
export  class LexerActionExecutor extends JavaObject {
	...
	private readonly  lexerActions:  LexerAction[] | null;
	/**
...
this.lexerActions = lexerActions;
	 ...
	 */
	public static append = (lexerActionExecutor: LexerActionExecutor| null, lexerAction: LexerAction| null):  LexerActionExecutor | null => {
		if (lexerActionExecutor === null) {
			return new  LexerActionExecutor( [ lexerAction ]);
		}

		let  lexerActions: LexerAction[] = java.util.Arrays.copyOf(lexerActionExecutor.lexerActions, lexerActionExecutor.lexerActions.length + 1);
	 ...
	 */
	public fixOffsetBeforeMatch = (offset: number):  LexerActionExecutor | null => {
		let  updatedLexerActions: LexerAction[] = null;
		for (let  i: number = 0; i < this.lexerActions.length; i++) {
	 ...
	 * of the token.
	 */
	public execute = (lexer: Lexer| null, input: CharStream| null, startIndex: number):  void => {
		let  requiresSeek: boolean = false;
		let  stopIndex: number = input.index();
		try {
			for (let lexerAction of this.lexerActions) {
				if (lexerAction instanceof LexerIndexedCustomAction) {
					let  offset: number = (lexerAction as LexerIndexedCustomAction).getOffset();
	...
	public equals = (obj: java.lang.Object| null):  boolean => {
		...
		let  other: LexerActionExecutor = obj as LexerActionExecutor;
