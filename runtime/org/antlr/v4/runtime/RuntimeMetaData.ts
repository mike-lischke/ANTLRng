...
export class RuntimeMetaData extends JavaObject {
     ...
     * </ul>
     */
    public static readonly VERSION: java.lang.String = S`4.11.2-SNAPSHOT`;
     ...
     * to {@link #VERSION}.
     */
    public static checkVersion = (generatingToolVersion: java.lang.String | null,
        compileTimeVersion: java.lang.String): void => {
        ...
        if (generatingToolVersion !== null) {
            runtimeConflictsWithGeneratingTool =
                ...
                !RuntimeMetaData.getMajorMinorVersion(runtimeVersion)
                    .equals(RuntimeMetaData.getMajorMinorVersion(generatingToolVersion));
        }

        runtimeConflictsWithCompileTimeTool =
            !runtimeVersion.equals(compileTimeVersion) &&
            !RuntimeMetaData.getMajorMinorVersion(runtimeVersion).equals(RuntimeMetaData
                .getMajorMinorVersion(compileTimeVersion));

        if (runtimeConflictsWithGeneratingTool) {
            java.lang.System.err.printf(
                S`ANTLR Tool version %s used for code generation does not match the current runtime version %s%n`,
                generatingToolVersion, runtimeVersion);
        }
        if (runtimeConflictsWithCompileTimeTool) {
            java.lang.System.err.printf(
                S`ANTLR Runtime version %s used for parser compilation does not match the current runtime version %s%n`,
     ...
     */
    public static getMajorMinorVersion = (version: java.lang.String): java.lang.String => {
        const dot = S`.`;
        const dash = S`-`;

        const firstDot = version.indexOf(dot);
        const secondDot = firstDot >= 0 ? version.indexOf(dot, firstDot + 1) : -1;
        const firstDash = version.indexOf(dash);
