...
export  class TokenVocabParser {
	...
	public  load():  Map<string,number> {
		...
		let  maxTokenType = -1;
		let  fullFile = this.getImportedVocabFile();
		let  fis = null;
		let  br = null;
		let  tool = this.g.tool;
		let  vocabName = this.g.getOptionString("tokenVocab");
		try {
			let  tokenDefPattern = Pattern.compile("([^\n]+?)[ \\t]*?=[ \\t]*?([0-9]+)");
			...
			let  lineNum = 1;
			while ( tokenDef!==null ) {
				let  matcher = tokenDefPattern.matcher(tokenDef);
				if ( matcher.find() ) {
					let  tokenID = matcher.group(1);
					let  tokenTypeS = matcher.group(2);
...
if (fnfeOrE instanceof FileNotFoundException) {
			const fnfe = fnfeOrE;
let  inTree = this.g.ast.getOptionAST("tokenVocab");
			let  inTreeValue = inTree.getToken().getText();
