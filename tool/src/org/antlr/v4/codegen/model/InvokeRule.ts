...
export  class InvokeRule extends RuleElement implements LabeledOp {
	...
	@ModelElement
public  argExprsChunks:  Array<ActionChunk>;
	...
	public  constructor(factory: ParserFactory, ast: GrammarAST, labelAST: GrammarAST) {
		...
		let  target = gen.getTarget();
		let  identifier = ast.getText();
		let  r = factory.getGrammar().getRule(identifier);
		this.name = r.name;
		...
		if ( labelAST!==null ) {
			...
			if ( labelAST.parent.getType() === ANTLRParser.PLUS_ASSIGN  ) {
				...
				let  listLabel = gen.getTarget().getListLabel(label);
		...
		let  arg = ast.getFirstChildWithType(ANTLRParser.ARG_ACTION) as ActionAST;
		if ( arg !== null ) {
		...
		if ( factory.getCurrentOuterMostAlt().ruleRefsInActions.containsKey(identifier) ) {
			let  label = gen.getTarget().getImplicitRuleLabel(identifier);
			let  d = new  RuleContextDecl(factory,label,this.ctxName);
			this.labels.add(d);
