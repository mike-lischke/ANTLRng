...
public  finishPrequels(firstPrequel: GrammarAST):  void {
 ...
 return;
		...
		let  parent = firstPrequel.parent as GrammarAST;
		let  options = parent.getAllChildrenWithType(OPTIONS);
		let  imports = parent.getAllChildrenWithType(IMPORT);
		let  tokens = parent.getAllChildrenWithType(TOKENS_SPEC);
...
public  discoverRule(rule: RuleAST, ID: GrammarAST,
							 modifiers: Array<GrammarAST>,
							 arg: ActionAST, returns: ActionAST,
							 thrws: GrammarAST, options: GrammarAST,
							 locals: ActionAST,
							 actions: Array<GrammarAST>, block: GrammarAST):  void
	{
...
public  finishRule(rule: RuleAST, ID: GrammarAST, block: GrammarAST):  void {
 ...
 return;
		...
		let  blk = rule.getFirstChildWithType(BLOCK) as BlockAST;
		let  nalts = blk.getChildCount();
		let  idAST = rule.getChild(0) as GrammarAST;
		for (let  i=0; i< nalts; i++) {
			let  altAST = blk.getChild(i) as AltAST;
			if ( altAST.altLabel!==null ) {
				let  altLabel = altAST.altLabel.getText();
				// first check that label doesn't conflict with a rule
				// label X or x can't be rule x.
				let  r = this.ruleCollector.rules.get(Utils.decapitalize(altLabel));
				if ( r!==null ) {
				...
				let  prevRuleForLabel = this.ruleCollector.altLabelToRuleName.get(altLabel);
		...
		let  altLabels = this.ruleCollector.ruleToAltLabels.get(rule.getRuleName());
		let  numAltLabels = 0;
		if ( altLabels!==null ) {
...
public  actionInAlt(action: ActionAST):  void {
		if (this.inFragmentRule) {
			let  fileName = action.token.getInputStream().getSourceName();
			let  ruleName = currentRuleName;
...
public  label(op: GrammarAST, ID: GrammarAST, element: GrammarAST):  void {
		...
		default:{
			let  fileName = ID.token.getInputStream().getSourceName();
...
protected  enterChannelsSpec(tree: GrammarAST):  void {
	...
	protected  checkGrammarName(nameToken: Token): void {
		let  fullyQualifiedName = nameToken.getInputStream().getSourceName();
		if (fullyQualifiedName === null) {
			// This wasn't read from a file.
			return;
		}

		let  f = new  File(fullyQualifiedName);
 ...
 // don't warn about diff if this is implicit lexer
	...
	protected  checkNumRules(rulesNode: GrammarAST): void {
		if ( rulesNode.getChildCount()===0 ) {
			let  root = rulesNode.getParent() as GrammarAST;
			let  IDNode = root.getChild(0) as GrammarAST;
			this.g.tool.errMgr.grammarError(ErrorType.NO_RULES, this.g.fileName,
					null, IDNode.getText(), this.g);
		}
	}

	protected  checkNumPrequels(options: Array<GrammarAST>,
						  imports: Array<GrammarAST>,
						  tokens: Array<GrammarAST>): void
	{
		let  secondOptionTokens = new  Array<Token>();
		...
		for (let t of secondOptionTokens) {
			let  fileName = t.getInputStream().getSourceName();
	...
	protected  checkInvalidRuleRef(ruleID: Token): void {
		let  fileName = ruleID.getInputStream().getSourceName();
		if ( this.g.isLexer() && Character.isLowerCase(ruleID.getText().charAt(0)) ) {
	...
	protected  checkTokenDefinition(tokenID: Token): void {
		let  fileName = tokenID.getInputStream().getSourceName();
...
protected  enterLexerCommand(tree: GrammarAST):  void {
		...
		if (this.inFragmentRule) {
			let  fileName = tree.token.getInputStream().getSourceName();
			let  ruleName = currentRuleName;
	...
	protected  checkElementIsOuterMostInSingleAlt(tree: GrammarAST):  void {
		let  alt = tree.parent;
		let  blk = alt.parent;
		let  outerMostAlt = blk.parent.getType() === RULE;
		let  rule = tree.getAncestor(RULE);
		let  fileName = tree.getToken().getInputStream().getSourceName();
		if ( !outerMostAlt || blk.getChildCount()>1 )
		{
			let  e = ErrorType.LEXER_COMMAND_PLACEMENT_ISSUE;
...
default:
		...
		let  optionName = optionID.getText();
								...
								valueAST: GrammarAST): boolean
	{
		if (this.checkAssocElementOption && ID !== null && "assoc".equals(ID.getText())) {
			if (elem.getType() !== ANTLRParser.ALT) {
				let  optionID = ID.token;
				let  fileName = optionID.getInputStream().getSourceName();
		...
		if ( elem instanceof RuleRefAST ) {
			return this.checkRuleRefOptions(elem as RuleRefAST, ID, valueAST);
		}
		if ( elem instanceof TerminalAST ) {
			return this.checkTokenOptions(elem as TerminalAST, ID, valueAST);
		}
	...
	protected  checkImport(importID: Token): void {
		let  delegate = this.g.getImportedGrammar(importID.getText());
		if ( delegate===null ) {
 return;
}

		let  validDelegators = BasicSemanticChecks.validImportTypes.get(delegate.getType());
	...
	private  checkCaseInsensitiveOption(optionID: Token, valueAST: GrammarAST, parentType: number):  void {
		...
		if (optionName.equals(Grammar.caseInsensitiveOptionName)) {
			let  valueText = valueAST.getText();
			if (valueText.equals("true") || valueText.equals("false")) {
				let  currentValue = Boolean.parseBoolean(valueText);
