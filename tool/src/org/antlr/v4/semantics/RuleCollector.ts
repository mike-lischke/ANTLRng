...
public  discoverRule(rule: RuleAST, ID: GrammarAST,
							 ...
							 block: GrammarAST):  void
	{
		...
		for (let a of actions) {
			// a = ^(AT ID ACTION)
			let  action =  a.getChild(1) as ActionAST;
...
public  discoverOuterAlt(alt: AltAST):  void {
		if ( alt.altLabel!==null ) {
			...
			let  altLabel = alt.altLabel.getText();
...
public  grammarOption(ID: GrammarAST, valueAST: GrammarAST):  void {
		let  caseInsensitive = this.getCaseInsensitiveValue(ID, valueAST);
	...
	@Override
public  discoverLexerRule(rule: RuleAST, ID: GrammarAST, modifiers: Array<GrammarAST>,
								  options: GrammarAST, block: GrammarAST):  void
	{
		let  currentCaseInsensitive = this.grammarCaseInsensitive;
		if (options !== null) {
			for (let child of options.getChildren()) {
				let  childAST =  child as GrammarAST;
				let  caseInsensitive = this.getCaseInsensitiveValue(childAST.getChild(0) as GrammarAST, childAST.getChild(1) as GrammarAST);
				if (caseInsensitive !== null) {
		...
		let  numAlts = block.getChildCount();
		let  r = new  Rule(this.g, ID.getText(), rule, numAlts, currentModeName, currentCaseInsensitive);
