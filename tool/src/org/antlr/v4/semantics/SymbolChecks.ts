...
export  class SymbolChecks {
	...
	public  constructor(g: Grammar, collector: SymbolCollector) {
		...
		for (let tokenId of collector.tokenIDRefs) {
 ...
 this.nameToRuleMap.put(r.name, r);
	...
	public  checkActionRedefinitions(actions: Array<GrammarAST>):  void {
		if (actions === null) {
 return;
}
		...
		for (let ampersandAST of actions) {
	...
	public  checkForLabelConflicts(rules: Collection<Rule>):  void {
		for (let r of rules) {
			...
			let  labelNameSpace = new  HashMap();
			for (let  i = 1; i <= r.numberOfAlts; i++) {
				let  a = r.alt[i];
				for (let pairs of a.labelDefs.values()) {
					if (r.hasAltSpecificContexts()) {
						// Collect labelName-labeledRules map for rule with alternative labels.
						let  labelPairs = new  HashMap();
						for (let p of pairs) {
							let  labelName = this.findAltLabelName(p.label);
							if (labelName !== null) {
								let  list: Array<LabelElementPair>;
						...
						for (let internalPairs of labelPairs.values()) {
	...
	public  checkForModeConflicts(g: Grammar):  void {
		if (g.isLexer()) {
			let  lexerGrammar = g as LexerGrammar;
			for (let modeName of lexerGrammar.modes.keySet()) {
				if (!modeName.equals("DEFAULT_MODE") && this.reservedNames.contains(modeName)) {
					let  rule = lexerGrammar.modes.get(modeName).iterator().next();
	...
	public  checkForUnreachableTokens(g: Grammar):  void {
		if (g.isLexer()) {
			let  lexerGrammar = g as LexerGrammar;
			for (let rules of lexerGrammar.modes.values()) {
				// Collect string literal lexer rules for each mode
				let  stringLiteralRules = new  Array();
				let  stringLiteralValues = new  Array();
				for (let  i = 0; i < rules.size(); i++) {
					let  rule = rules.get(i);

					let  ruleStringAlts = this.getSingleTokenValues(rule);
				...
				for (let  i = 0; i < stringLiteralRules.size(); i++) {
					let  firstTokenStringValues = stringLiteralValues.get(i);
					let  rule1 =  stringLiteralRules.get(i);
					...
					if (!rule1.isFragment()) {
						for (let  j = i + 1; j < stringLiteralRules.size(); j++) {
							let  rule2 = stringLiteralRules.get(j);
							if (!rule2.isFragment()) {
	...
	public  checkRuleArgs(g: Grammar, rulerefs: Array<GrammarAST>):  void {
		if ( rulerefs===null ) {
 return;
}

		for (let ref of rulerefs) {
			let  ruleName = ref.getText();
			let  r = g.getRule(ruleName);
			let  arg = ref.getFirstChildWithType(ANTLRParser.ARG_ACTION) as GrammarAST;
 ...
 if ( arg===null && (r!==null && r.args!==null) ) {
	...
	public  checkForQualifiedRuleIssues(g: Grammar, qualifiedRuleRefs: Array<GrammarAST>):  void {
		for (let dot of qualifiedRuleRefs) {
			let  grammar = dot.getChild(0) as GrammarAST;
			let  rule = dot.getChild(1) as GrammarAST;
			g.tool.log("semantics", grammar.getText()+"."+rule.getText());
			let  delegate = g.getImportedGrammar(grammar.getText());
			if ( delegate===null ) {
	...
	protected  checkDeclarationRuleConflicts(r: Rule, attributes: AttributeDict, ruleNames: Set<string>, errorType: ErrorType):  void {
		...
		for (let attribute of attributes.attributes.values()) {
	...
	protected  checkLocalConflictingDeclarations(r: Rule, attributes: AttributeDict, referenceAttributes: AttributeDict, errorType: ErrorType):  void {
		...
		let  conflictingKeys = attributes.intersection(referenceAttributes);
	...
	private  checkLabelPairs(r: Rule, labelNameSpace: Map<string, LabelElementPair>, pairs: Array<LabelElementPair>):  void {
		for (let p of pairs) {
			this.checkForLabelConflict(r, p.label);
			let  name = p.label.getText();
			let  prev = labelNameSpace.get(name);
			if (prev === null) {
 ...
 if (altAST.leftRecursiveAltInfo !== null) {
	...
	private  checkForTypeMismatch(r: Rule, prevLabelPair: LabelElementPair, labelPair: LabelElementPair):  void {
		...
		if (prevLabelPair.type !== labelPair.type) {
			...
			let  token = r instanceof LeftRecursiveRule
		...
		if (!prevLabelPair.element.getText().equals(labelPair.element.getText()) &&
			...
			let  token = r instanceof LeftRecursiveRule
					? ( r.ast.getChild(0) as GrammarAST).getToken()
					: labelPair.label.token;
			let  prevLabelOp = prevLabelPair.type.equals(LabelType.RULE_LIST_LABEL) ? "+=" : "=";
			let  labelOp = labelPair.type.equals(LabelType.RULE_LIST_LABEL) ? "+=" : "=";
			this.errMgr.grammarError(
					ErrorType.LABEL_TYPE_CONFLICT,
					this.g.fileName,
					token,
	 ...
	 */
	private  getSingleTokenValues(rule: Rule):  Array<string>
	{
		let  values = new  Array();
		for (let alt of rule.alt) {
			if (alt !== null) {
				// select first alt if token has a command
				let  rootNode = alt.ast.getChildCount() === 2 &&
				...
				for (let  i = 0; i < rootNode.getChildCount(); i++) {
					...
					let  terminalAST = child as TerminalAST;
					if (terminalAST.token.getType() !== ANTLRLexer.STRING_LITERAL) {
						ignore = true;
						break;
					}
					else {
						let  text = terminalAST.token.getText();
	 ...
	 * TOKEN2: 'asdf';
	 */
	private  checkForOverlap(g: Grammar, rule1: Rule, rule2: Rule, firstTokenStringValues: Array<string>, secondTokenStringValues: Array<string>):  void {
		for (let  i = 0; i < firstTokenStringValues.size(); i++) {
			let  secondTokenInd = rule1 === rule2 ? i + 1 : 0;
			let  str1 = firstTokenStringValues.get(i);
			for (let  j = secondTokenInd; j < secondTokenStringValues.size(); j++) {
				let  str2 = secondTokenStringValues.get(j);
				if (str1.equals(str2)) {
