...
public check(...args: unknown[]):  void |  boolean {
		...
		for (let start of this.atn.ruleToStartState) {
 ...
 return false;
		...
		for (let  i=0; i<n; i++) {
			...
			if ( t instanceof RuleTransition ) {
				let  rt =  t as RuleTransition;
				let  r = this.g.getRule(rt.ruleIndex);
				if ( this.rulesVisitedPerRuleCheck.contains(t.target as RuleStartState) ) {
					...
					let  nullable = this.check(r, t.target, new  HashSet<ATNState>());
 ...
 if ( t.isEpsilon() ) {
	...
	protected  addRulesToCycle(enclosingRule: Rule, targetRule: Rule):  void {
		...
		for (let rulesInCycle of this.listOfRecursiveCycles) {
		...
		if ( !foundCycle ) {
			let  cycle = new  OrderedHashSet<Rule>();
