...
import { HashMap, IntervalSet } from "antlr4ng";
...
export  class LeftRecursiveRuleAnalyzer extends LeftRecursiveRuleWalker {
	...
	public  altAssociativity = new  Map<number, LeftRecursiveRuleAnalyzer.ASSOC>();
 ...
 return false;
		...
		let  blk = t.getFirstChildWithType(BLOCK) as GrammarAST;
		if ( blk===null ) {
 return false;
}

		let  n = blk.getChildren().size();
		for (let  i = 0; i < n; i++) {
			let  alt = blk.getChildren().get(i) as GrammarAST;
...
public  setAltAssoc(t: AltAST, alt: number):  void {
		let  assoc = LeftRecursiveRuleAnalyzer.ASSOC.left;
 ...
 if ( a.equals(LeftRecursiveRuleAnalyzer.ASSOC.left.toString()) ) {
					assoc = LeftRecursiveRuleAnalyzer.ASSOC.left;
				}
				else {
					this.tool.errMgr.grammarError(ErrorType.ILLEGAL_OPTION_VALUE, t.g.fileName, t.getOptionAST("assoc").getToken(), "assoc", assoc);
				}
}
...
public  binaryAlt(originalAltTree: AltAST, alt: number):  void {
...
public  prefixAlt(originalAltTree: AltAST, alt: number):  void {
		let  altTree = originalAltTree.dupTree() as AltAST;
		this.stripAltLabel(altTree);
...
public  suffixAlt(originalAltTree: AltAST, alt: number):  void {
		let  altTree = originalAltTree.dupTree() as AltAST;
		let  altLabel = altTree.altLabel!==null ? altTree.altLabel.getText() : null;
...
public  otherAlt(originalAltTree: AltAST, alt: number):  void {
		let  altTree = originalAltTree.dupTree() as AltAST;
		this.stripAltLabel(altTree);
		let  altText = this.text(altTree);
		let  altLabel = altTree.altLabel!==null ? altTree.altLabel.getText() : null;
...
//		System.out.println("otherAlt " + alt + ": " + altText);
 ...
 return null;
		...
		let  outerAltRuleRefs = t.getNodesWithTypePreorderDFS(IntervalSet.of(RULE_REF));
	...
	public  stripLeftRecursion(altAST: GrammarAST):  GrammarAST {
		...
		if ( (first.getType()===RULE_REF && first.getText().equals(this.ruleName)) ||
			 (rref!==null && rref.getType()===RULE_REF && rref.getText().equals(this.ruleName)) )
		{
 ...
 lrlabel = first.getChild(0) as GrammarAST;
	...
	public  stripAltLabel(altAST: GrammarAST):  void {
		...
		for (let  i=stop; i>=start; i--) {
			if ( this.tokenStream.get(i).getType()===POUND ) {
 ...
 return "";
		...
		let  optionsSubTrees = t.getNodesWithType(ELEMENT_OPTIONS);
		...
		let  labeledSubTrees = t.getNodesWithType(new  IntervalSet(ASSIGN,PLUS_ASSIGN));
		...
		while ( i<=tokenStopIndex ) {
			...
			if ( tok.getType()===RULE_REF && i<=tokenStopIndex && this.tokenStream.get(i).getType()===ARG_ACTION ) {
	...
	public  nextPrecedence(alt: number):  number {
		...
		if ( this.altAssociativity.get(alt)===LeftRecursiveRuleAnalyzer.ASSOC.right ) {
 return p;
}
...
public  toString():  string {
	...
	public static  ASSOC =  class ASSOC extends Enum<ASSOC> { public static readonly left: ASSOC = new class extends ASSOC {
...
export namespace LeftRecursiveRuleAnalyzer {
	export type ASSOC = InstanceType<typeof LeftRecursiveRuleAnalyzer.ASSOC>;
}
