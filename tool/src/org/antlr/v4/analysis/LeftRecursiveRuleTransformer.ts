...
export  class LeftRecursiveRuleTransformer {
	...
	public  translateLeftRecursiveRules():  void {
		let  language = this.g.getLanguage();
		// translate all recursive rules
		let  leftRecursiveRuleNames = new  Array<string>();
		for (let r of this.rules) {
			if ( !Grammar.isTokenName(r.name) ) {
				if ( LeftRecursiveRuleAnalyzer.hasImmediateRecursiveRuleRefs(r.ast, r.name) ) {
					let  fitsPattern = this.translateLeftRecursiveRule(this.ast, r as LeftRecursiveRule, language);
					if ( fitsPattern ) {
		...
		for (let r of this.ast.getNodesWithType(ANTLRParser.RULE_REF)) {
 ...
 // already has arg; must be in rewritten rule
			if ( leftRecursiveRuleNames.contains(r.getText()) ) {
				...
				(r as GrammarASTWithOptions).setOption(LeftRecursiveRuleTransformer.PRECEDENCE_OPTION_NAME, new  GrammarASTAdaptor().create(ANTLRParser.INT, "0") as GrammarAST);
			}
		}
	}
											  ...
											  language: string):  boolean
	{
		...
		let  prevRuleAST = r.ast;
		let  ruleName = prevRuleAST.getChild(0).getText();
		let  leftRecursiveRuleWalker =
...
if (re instanceof RecognitionException) {
 ...
 return false;
		...
		let  RULES = ast.getFirstChildWithType(ANTLRParser.RULES) as GrammarAST;
		let  newRuleText = leftRecursiveRuleWalker.getArtificialOpPrecRule();
//		System.out.println("created: "+newRuleText);
		...
		let  t = this.parseArtificialRule(prevRuleAST.g, newRuleText);
		...
		let  transform = new  GrammarTransformPipeline(this.g, this.g.tool);
		transform.reduceBlocksToSets(r.ast);
		transform.expandParameterizedLoops(r.ast);

		// Rerun semantic checks on the new rule
		let  ruleCollector = new  RuleCollector(this.g);
		ruleCollector.visit(t, "rule");
		let  basics = new  BasicSemanticChecks(this.g, ruleCollector);
		...
		let  arg = r.ast.getFirstChildWithType(ANTLRParser.ARG_ACTION) as ActionAST;
		if ( arg!==null ) {
		...
		for (let pair of leftRecursiveRuleWalker.leftRecursiveRuleRefLabels) {
			let  labelNode = pair.a;
			let  labelOpNode = labelNode.getParent() as GrammarAST;
			let  elementNode = labelOpNode.getChild(1) as GrammarAST;
			let  lp = new  LabelElementPair(this.g, labelNode, elementNode, labelOpNode.getType());
	...
	public  parseArtificialRule(/* final */  g: Grammar, ruleText: string):  RuleAST {
		let  lexer = new  ANTLRLexer(new  ANTLRStringStream(ruleText));
		let  adaptor = new  GrammarASTAdaptor(lexer.getCharStream());
		let  tokens = new  CommonTokenStream(lexer);
		lexer.tokens = tokens;
		let  p = new  ToolANTLRParser(tokens, this.tool);
		p.setTreeAdaptor(adaptor);
		let  ruleStart = null;
		try {
			let  r = p.rule();
			let  tree = r.getTree() as RuleAST;
...
//		System.out.println("RULE: "+t.toStringTree());
		let  ruleBlk = t.getFirstChildWithType(ANTLRParser.BLOCK) as BlockAST;
		let  mainAlt = ruleBlk.getChild(0) as AltAST;
		let  primaryBlk = mainAlt.getChild(0) as BlockAST;
		let  opsBlk = mainAlt.getChild(1).getChild(0) as BlockAST; // (* BLOCK ...)
