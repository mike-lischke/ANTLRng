...
import { CharStream, CharStreams, CommonToken, CommonTokenStream, DiagnosticErrorListener, Lexer, Parser, ParserRuleContext, Token, TokenStream, PredictionMode } from "antlr4ng";
...
if (cnfe2 instanceof ClassNotFoundException) {
	...
	throw cnfe;
		...
		let  lexerCtor = lexerClass.getConstructor(CharStream.class);
		let  lexer = lexerCtor.newInstance(null as CharStream);

		let  parserClass = null;
		let  parser = null;
		if ( !this.startRuleName.equals(TestRig.LEXER_START_RULE_NAME) ) {
			let  parserName = this.grammarName+"Parser";
			parserClass = cl.loadClass(parserName).asSubclass(Parser.class);
			let  parserCtor = parserClass.getConstructor(TokenStream.class);
		...
		if ( this.inputFiles.size()===0 ) {
			let  charStream = CharStreams.fromStream(System.in, charset);
			this.process(lexer, parserClass, parser, charStream);
			return;
		}
		for (let inputFile of this.inputFiles) {
	                let  charStream = CharStreams.fromPath(Paths.get(inputFile), charset);
			...
			if ( this.showTokens ) {
				for (let tok of tokens.getTokens()) {
					if ( tok instanceof CommonToken ) {
						System.out.println((tok as CommonToken).toString(lexer));
					}
					else {
 ...
 return;
			...
			try {
				let  startRule = parserClass.getMethod(this.startRuleName);
				let  tree = startRule.invoke(parser, null as Object[]) as ParserRuleContext;
