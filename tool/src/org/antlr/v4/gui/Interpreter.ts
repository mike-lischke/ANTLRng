...
export  class Interpreter {
    ...
    protected  interp():  ParseInfo {
        ...
        if (this.grammarFileName !== null) {
            const  grammarContent = Files.readString(Path.of(this.grammarFileName));
            g = new  Interpreter.IgnoreTokenVocabGrammar(this.grammarFileName, grammarContent, null, listener);
        }
        else {
            const  lexerGrammarContent = Files.readString(Path.of(this.lexerGrammarFileName));
            lg = new  LexerGrammar(lexerGrammarContent, listener);
            const  parserGrammarContent = Files.readString(Path.of(this.parserGrammarFileName));
        ...
        if ( this.showTokens ) {
            for (const tok of tokens.getTokens()) {
                if ( tok instanceof CommonToken ) {
                    System.out.println((tok).toString(lexEngine));
                }
                else {
        ...
        const  r = g.rules.get(this.startRuleName);
        if (r === null) {
    ...
    private  dumpProfilerCSV(parser: GrammarParserInterpreter, parseInfo: ParseInfo):  void {
        const  ruleNamesByDecision = new  Array<string>(parser.getATN().decisionToState.size());
        ...
        try {
            const  fileWriter = new  FileWriter(this.profileFileName);
            const  pw = new  PrintWriter(fileWriter);
